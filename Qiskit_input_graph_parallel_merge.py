...
def draw_binary_tree(Bt):
    ...
    pos = nx.drawing.nx_pydot.graphviz_layout(Bt, prog='dot')
...
def main():
    ...
    _,msq,_= calculate_msq(generate_ibm_graph(nodes_list, edges_list, use_barabasi=False))
    for i in range(len(msq)):   
      draw_graph(msq[i])
    
    bt,_ = parallel_merge(generate_ibm_graph(nodes_list, edges_list, use_barabasi=False),msq)
